...
class StrategyManager:
    ...
    def record_trade_result(
        ...
        """
        Record trade result for self-learning performance engine
        ...
        Args:
            ...
            regime: Market regime when entered
            session: Trading session
        """
        # Forward to swing strategy's performance engine
        swing_strategy = self.strategies.get('swing')
        if swing_strategy and hasattr(swing_strategy, 'record_trade_result'):
            # Convert duration to minutes
            duration_minutes = duration_seconds // 60 if duration_seconds else 0
            
            swing_strategy.record_trade_result(
                symbol=symbol,
                direction=direction,
                ...
                signal_score=0,  # Not available at close time
                session=session or 'unknown',
                duration_minutes=duration_minutes
            )
            logger.info(f"📈 Performance Engine: Recorded {symbol} trade - {'+' if pnl >= 0 else ''}{pnl_percent:.2f}%")
